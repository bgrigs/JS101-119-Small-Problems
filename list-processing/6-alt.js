...
function makeSubstrings(string) {
  return string.split("").map((_, idx) => makeLeadingSubstrings(string, idx)).flat();
}

function makeLeadingSubstrings(string, idx) {
  let substrings = [];
  let substring = '';

  for (let length = idx; length < string.length; length++) {
