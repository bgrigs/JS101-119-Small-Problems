...
function isValidName(name) {
  return (/^[a-z]+ [a-z]+$/i).test(name); // was (/^\w+ \w+$/).test(name);
}
...
LS: 
Discussion
Recall that when using regular expressions, the \w character class shortcut matches not only alphabetic characters (a-z, A-Z), but also digits (0-9) and the underscore (_). This isn't the behavior intended for our code.
...
Further Exploration
Check out Scriptular if you want to test or to experiment with JavaScript regular expressions.
*/